...
let monthNamesShort = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'June', 'July', 'Aug', 'Sept', 'Oct', 'Nov', 'Dec'];
let dayNames = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
let dayNamesShort = ['Sun', 'Mon', 'Tues', 'Wed', 'Thur', 'Fri', 'Sat'];
let adjustedDayNames = [];
let adjustedDayNamesShort = [];
let weekStartsOn = 0;
...
const dateHelper = {
  ...
  create: function(date) {
    ...
    let inst = Object.create(dateHelper);
    inst._date = date;
    inst._cache = {};
  ...
  getDayOfWeek: function(date) {
    let day = (date instanceof Date ? date : new Date(date.year, date.month - 1, date.day)).getDay() - weekStartsOn;
  ...
  getWeekStart: function(date) {
    let inst = dateHelper.create(new Date(date.year, date.month - 1, date.day - dateHelper.getDayOfWeek(date) + 1));
  ...
  getMonthStart: function(date) {
    let inst = dateHelper.create(new Date(date.year, date.month - 1, 1));
    return inst;
  },
  ...
  getNextWeek: function(date) {
    let start = dateHelper.getWeekStart(date);
  ...
  getPreviousWeek: function(date) {
    let start = dateHelper.getWeekStart(date);
    let inst = dateHelper.create(new Date(start.year, start.month - 1, start.day - 7));
    inst.weekStartsOn = weekStartsOn;
    return inst;
  },
  ...
  equalDay: function(date, compare, full) {
    ...
    let args = this._checkComparisonArgs(date, compare, full);
    date = args[0];
    compare = args[1];
    full = args[2];
